...
def createmi():
    os.system('az sql mi create -g ' + rgname + ' -n ' + servername + ' -l ' + location + ' -i -u ' + dbadmin + ' -p ' + kvsecret + ' --public-data-endpoint-enabled ' + pubendpoint + ' --license-type BasePrice --subnet /subscriptions/' + subid + '/resourceGroups/' + rgname + '/providers/Microsoft.Network/virtualNetworks/vnet-' + servername + '/subnets/subnet' + servername + ' --capacity 4 --storage 256GB --edition GeneralPurpose --family Gen5 --minimal-tls-version 1.2 --proxy-override "Redirect" --no-wait --tags BU="' + BU + '" DU="' + DU + '" Environment="' + Environment + '" SDU="' + SDU + '" Product="' + Product + '" "Cost Center Code"="' + CostCenterCode + '" Hierarchy="' + Hierarchy + '" "Industry Specialization"="' + IndustrySpecialization + '" "Project Code"="' + ProjectCode + '" "Project Sponsor"="' + ProjectSponsor + '" "Technical Owner"="' + TechnicalOwner + '"')
