...
data "aws_secretsmanager_secret" "_secret" {
  name = var._secret_name
}

data "aws_secretsmanager_secret_version" "_secret_version" {
  secret_id     = data.aws_secretsmanager_secret._secret.id
}
...
module "ec2_instance" {
  ...
  pass               = data.aws_secretsmanager_secret_version._secret_version.secret_string
