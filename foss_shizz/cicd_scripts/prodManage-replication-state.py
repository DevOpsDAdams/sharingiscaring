...
WaveSSMName = '<<secret_1>>'
ReplicationActionSSMName = '<<secret_2>>'
UserSSMName = '<<secret_3>>'
LoginPassSecretName = '<<secret_4>>'
CETokenSecretName = '<<secret_5>>'
ceServerList = '<<secret_6>>'
proj_id = []
ServerList = []
machineData = []
ServerNames = []
            ...
            for sepl in seplist:
                if sepl in ServerID:
                    ...
                    if ReplicationAction.lower() == "stop":
                        '''
                        print("Stopping the Replication is NOT advised at this time. Please reach out to your Systems Administrator to have this action performed")
                        sys.exit(0)
                        '''
