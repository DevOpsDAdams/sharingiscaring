...
resource "azurerm_storage_container" "blob_containers" {
  count                   = length(local.blob_containers)
  name                    = local.blob_containers[count.index]
  storage_account_name    = data.external.storage_account.result["Name"]
  container_access_type   = "private"
  depends_on = [
    module.storage.storage_account
  ]
  lifecycle {
    ignore_changes = [
