...
data "azurerm_subnet" "lb" {
  provider            = azurerm.networking
  name                 = var.json.common_network_info.lb_subnet
  virtual_network_name = var.json.common_network_info.vnet
  resource_group_name  = var.json.common_network_info.vnet_rg
}
...
data "azurerm_subnet" "agw" {
  provider            = azurerm.networking
  name                 = var.json.common_network_info.agw_subnet
  virtual_network_name = var.json.common_network_info.vnet
  resource_group_name  = var.json.common_network_info.vnet_rg
}
...
data "azurerm_network_interface" "nic" {
  ...
  name                = "${local.nic_prefix}${var.json.hyb_info[count.index].unique_name}0${count.index+1}-nic01"
